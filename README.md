...
### Setup without pre-configured package
...
Steps from the above could be automated with the selenium scripts, automation scripts live in the
"setup" folder.
...
To generate new certificates, go to the ```volumes/cert```, remove *crt, *key and *p12 files then
run ```bash gencert.sh <domain>```.  Certificates are used only for tests and there is no need to
worry about exposing them.
